...
static uint8 gps_gngga_parse(char* line, gnss_info_struct* gnss)
{
    ...
    if (',' != state)
    {
        ...
        gnss->height =
            get_float_number(&buf[get_parameter_index(9, buf)]) +
            get_float_number(&buf[get_parameter_index(11, buf)]);   // 高度 = 海拔高度 + 地球椭球面相对大地水准面的高度
        return_state = 1;
    }
...
double get_two_points_distance(double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    distance = 2 * asin(sqrt(pow(sin(a / 2), 2) + cos(rad_latitude1) * cos(rad_latitude2) *
                                                      pow(sin(b / 2), 2)));   // google maps 里面实现的算法
...
#endif   // DEFAULT_GPS_MATH_FLAG

#if GL_GPS_MATH_FLAG   // 古兰
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     计算从第一个点到第二个点的距离
// 参数说明     latitude1       第一个点的纬度
// 参数说明     longitude1      第一个点的经度
...
double get_two_points_distance(double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    distance = 2 * py_asin(func_sqrt(func_pow(func_sin(a / 2), 2) +
                                     func_cos(rad_latitude1) * func_cos(rad_latitude2) *
                                         func_pow(func_sin(b / 2), 2)));   // google maps 里面实现的算法
    distance = distance * EARTH_RADIUS;
...
double get_two_points_azimuth(double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    double y = func_cos(latitude1) * func_sin(latitude2) -
               func_sin(latitude1) * func_cos(latitude2) * func_cos(longitude2 - longitude1);
    double angle = RAD_TO_ANGLE(fast_atan2(x, y));

    return ((angle > 0) ? angle : (angle + 360));
}
#endif   // GL_GPS_MATH_FLAG
...
void gnss_uart_callback(void)
{
    ...
    if (gnss_state)
    {
        ...
        if ('\n' == dat)
        {
            ...
            gnss_flag = 1;
            enable_handler("gps");   // 使能GPS数据处理函数
        }
    }
}
...
{
    const uint8 set_rate[] = {0xF1, 0xD9, 0x06, 0x42, 0x14, 0x00, 0x00, 0x0A, 0x05, 0x00, 0x64, 0x00, 0x00, 0x00,
    ...
    if ((TAU1201 == gps_device) || (GN42A == gps_device))
    {
        ...
        uart_write_buffer(
            GNSS_UART, (uint8*)set_rate, sizeof(set_rate));   // 设置GPS更新速率为10hz 如果不调用此语句则默认为1hz
        system_delay_ms(200);
