...
static uint8  timer_running = 0;   // 标记是否已启动计时
//-------------------------------------------------------------------------------------------------------------------
//  函数简介      systick定时器启动
//  返回参数      void
...
//  返回参数      void
//  使用示例      timer_enable();
//  说明         记录当前时间作为计时起点（只使用系统提供的定时器基础函数）
//-------------------------------------------------------------------------------------------------------------------
void timer_enable(void)
{
    timer_start   = system_getval();
    timer_running = 1;
}

//-------------------------------------------------------------------------------------------------------------------
//  函数简介      结束计时并打印间隔时间（扩展函数）
//  返回参数      void
//  使用示例      timer_disable();
//  说明         获取当前计时时间，与 timer_enable 时记录的时间相减，计算时间差并以10ns、ms、us、ns单位打印输出
//-------------------------------------------------------------------------------------------------------------------
void timer_disable(void)
{
    if (!timer_running)
    {
        printf("错误：未启用定时器，请先调用 timer_enable()！\n");
        return;
    }

    uint32 current_val = system_getval();
    uint32 diff        = current_val - timer_start;

    printf("计时时间： %lu ms | %lu us\n",
           diff / 100000,   // ms
           diff / 100);     // us
